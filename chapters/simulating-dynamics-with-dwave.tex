...
\end{equation}
Here, $\mathcal{T}$ denotes the time-ordering operation. Note that in the case
when $K(t)$ commutes with $K(t')$ for every $t' \ne t$, the time-ordering can
be omitted. In particular, this is the case if $K$ is time-independent.
...
methods (e.g. via Suzuki-Trotter approximation).
...
\begin{equation}
  ...
  \braket{\mathbf{x}}{\Phi}.
\end{equation}
It is easy to verify that solution to \eqref{eq:gsys} also minimizes $h$ by
...
graph) while, on the other hand, it is a decreasing function of $L$ (larger
systems require sparser graphs).
...
2000Q\_2\_1 and D-Wave 2000Q\_5 machines to test the above-described approach.
