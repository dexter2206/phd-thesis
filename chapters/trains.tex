...
We will consider a part of a railway network, which we will simply refer to as a
\emph{network}. The network is divided into \emph{block sections} or simply \emph{blocks}.
In our approach, we focused only on the single--track railways, which means that the network
can only comprise the following types of blocks:
\begin{itemize}
...
\begin{figure}[ht]
  ...
  \caption{
    An example network. Sections $2, 3, 4$ and $6$ are \emph{line blocks}, while sections $1$ and
    $5$ are \emph{sidings} with respectively $4$ and $2$ tracks. Rectangles represent platforms.
    Circles represent points where a line block and a siding join (white) or where two line blocks
    join (blue). Superscripts denote tracks within a siding.
  }
  \label{fig:railway-network}
\end{figure}

The trains move through the network according to a \emph{timetable}. It is assumed that this
...
The distinction between primary and secondary delays might seem
...
The set of all trains will be denoted by $\JJ$. This set is naturally partitioned into
...
\begin{figure}
  ...
  \begin{subfigure}{\textwidth}
    ...
    \subcaption{
    Train diagram for the undisturbed timetable of the line in \textbf{(a)}. The timetable features two
...
\begin{figure}
  ...
  \begin{subfigure}{\textwidth}
    ...
    \subcaption{
    Train diagram for the timetable of the line in \textbf{(a)}.
...
For the Nidzica--Olsztynek railway segment, we considered two delays. The first one is a 15 minutes delay
...
In FCFS (resp. FLFS) the way is given to the train that first arrives (resp. first leaves) the
