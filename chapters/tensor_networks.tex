...
\todo{write more intro} While there exists a plethora of general-purpose
...
belonging to the considered cluster. \todo[inline,color=green]{Should we
  mention here, that $H(\mathbf{s_c})$ can be replaced by some approximation
  scheme?}
...
We start off by representing the probability distribution $\exp[-\beta
    ...
    given configuration $(s_1, \ldots, s_k)$. \todo[inline,color=SkyBlue]{Write
  marginal probability using conditional probability, show that this depends only
  on the boundary and not the whole searched configuration} To this end, with
...
\todo[color=magenta]{This is true, although maybe not necessary to mention
  here} \todo[inline,color=SkyBlue]{Mention that we shift from ising to factor
  graph}
