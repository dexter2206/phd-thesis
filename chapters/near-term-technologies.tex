...
\begin{figure}
  ...
  \caption{
    ...
    The grid layout. The internal couplers are marked with {\color{Tan} orange},
    and the external couplers are marked with {\color{RoyalBlue} blue}. }
  \label{fig:chimera}
\end{figure}
...
compiler\footnote{The \texttt{nvfortran} compiler was previously a third--party
  program called \texttt{pgfortran}, developed by the PGI \cite{PGI}}. Giving a
...
\begin{itemize}
  ...
  \item thrust \cite{thrust} -- parallel algorithms library. Interestingly, Thrust also
