...
J_{ij} \in \mathbb{R}$ are coefficients of the instance. Note that finding
the ground state of such Hamiltonian is equivalent to finding the ground state
of its classical counterpart (c.f. eq. \eqref{eq:ising-hamiltonian}). For small system sizes,
this can be accomplished by listing all possible configurations and sorting them
with respect to their energies.
...
\begin{figure}
  ...
  \caption{
    ...
    and the external couplers are marked with bold {\color{Tan} orange}. }
  \label{fig:chimera}
\end{figure}
...
\begin{example}[Minor embedding]
  ...
  \begin{figure}[b]
    ...
    \caption{
      ...
      (\textbf{a.}). By contracting neighboring vertices 4 and 0 one obtains a new logical graph
      $C_1'$ (\textbf{b.}), which contains problem graph $G$ as a subgraph. }
